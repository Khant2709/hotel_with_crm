...
const WrapperBooking = ({
  ...
  const filterBooking = useMemo(() => {
    ...
    const dateSelected = dateBooking ? new Date(dateBooking).toISOString()
        .split('T')[0] : null;

    return bookings
        .filter(booking =>
            (!id || booking.id === +id) &&
            (!guestName || booking.guest_fio.toLowerCase()
  ...
  return (
      <div className={styles.wrapperMain}>
        ...
        />
        <BookingTable hotels={hotels} apartments={apartments} bookings={filterBooking} isSettingPage={isSettingPage}/>
      </div>
  );
};
